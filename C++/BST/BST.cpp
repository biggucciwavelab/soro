...
int main(int argc, char* argv[]) {
	...
	collision::ChCollisionInfo::SetDefaultEffectiveCurvatureRadius(0.1f);  // Effective radius of curvature for all SCM contacts.
	collision::ChCollisionModel::SetDefaultSuggestedMargin(0.1f);
	collision::ChCollisionModel::SetDefaultSuggestedEnvelope(0.1f);
	...
	if (false) {
		...
		for (float torus = 1.25 * bot_angle / 2; torus < CH_C_2PI - 1.25*bot_angle; torus += bot_seperation + bot_angle) {
			...
			for (float radius = c - a; radius < r_o; radius += 3 * int_radius) {
				//float int_angle = 2 * tan(int_radius / radius);
				// At each height
				for (float height = -0.95*a; height < 0.95 * a; height += 3 * int_radius) {
					// TODO: Angles
					for (float u = tan(int_radius / radius) + torus; u < bot_seperation + torus - tan(int_radius / radius); u += 2 * tan(int_radius / radius)) {
						// Point on circle
		...
		for (float radius = r_i; radius < c - a; radius += 3 * int_radius) {
			...
			for (float height = -0.95 * a; height < 0.95 * a; height += 3 * int_radius) {
				// TODO: Angles
				for (float u = 0; u < CH_C_2PI- 2*tan(int_radius / radius); u += 2 * tan(int_radius / radius)) {
					// Point on circle
					float x = radius * cos(u);
					float y = height;
					float z = radius * sin(u);
...
//--------------------------------------
	...
	application.SetVideoframeSaveInterval(100);
...
//--------------------------------------
	...
	while (application.GetDevice()->run()) {
		...
		if (t > 0.2 || timer.GetTimeMillisecondsIntermediate()>6500) {
			total_timer.stop();
