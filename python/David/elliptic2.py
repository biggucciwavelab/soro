...
def reconstruct_series(coeffs, locus=(0), num_points=360):
    ...
    xt_all = xt_all + np.ones((num_points,)) * locus

    reconstruction = np.stack([t, xt_all], axis=1)
    return reconstruction

def fourier_series(data, order=10, normalize=False, return_transformation=False):
    dxy = np.diff(data[:, 1])
    dt = np.diff(data[:, 0])
    t = np.concatenate([([0.0]), np.cumsum(dt)])
    T = t[-1]  #
