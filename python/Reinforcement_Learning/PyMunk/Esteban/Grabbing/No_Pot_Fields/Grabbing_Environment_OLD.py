...
This is an OLD environment which contains information from both the square as well as the system. Future environment iterations do not contain square information
"""
...
class pymunkEnv(Env):
    ...
    def __init__(self, dt, ppm, screenHeight, screenWidth, 
        ...
        self.maxAppliedForce = 1.0                                   # The maximum force a bot can apply in any given direction
        ...
        # Square size
        self.squareLength = self.R * 2 
        # self.squareLength = self.R * 3 # Increase size of box. Use only if 1 bot is present!

        # Object Starting Position
        objectX = self.width - (self.squareLength/2 + wallThickness/2)
        ...
        self.systemStart = self.width - (R + self.squareLength + botRadius + wallThickness), self.height/2 # Start next to box
        ...
        # self.systemStart = self.width - (wallThickness/2+botRadius/2+R/2), self.height/4 # Start below box. Use only if 1 bot is present!

        # Location to move object to
        # self.targetLoc = R + wallThickness, self.height/2
        self.targetLoc = self.width/2, self.height/2
        self.targetDistance = np.linalg.norm(np.asarray(self.targetLoc) - np.asarray(self.objectPosition))
        ...
        #### Gym API
        self.state_size = self.numBots*8+ 4 + 1 # botRePosition (x,y), botVel (x,y), botCurrentAction(x,y), objectPos(x,y), botVel(x,y), timeLeft
    ...
    def reset(self):
        ...
        #### Add object to grab
        objectMass = self.botMass*self.numBots + self.skinMass*(self.skinRatio*self.numBots) + self.inMass*self.numInterior
        ...
        # Bound the object using springs
    ...
    def step(self, ac):
        ...
        # Resetting contact information on squarew
        self.extForceSquare[:] = 0
        ...
        for i in range(self.numStepsPerStep):
            ...
            # Move Square
            # self.square.body.apply_force_at_world_point((-1,0), squarePos)
        ...
        # Gather information
        obs, systemCenter, distanceToTarget, distanceToSquare = self.getOb(ac)
    ...
    def getOb(self, ac):
        ...
        distanceToTarget = np.linalg.norm(squareLocation) 

        # Calculating the distance to 
        distanceToSquare = np.linalg.norm(np.mean(botPos,axis=0))
        ...
        squareObsLocation = [squareLocation[0]/self.width, squareLocation[1]/self.height]
        squareVel = np.asarray(squareVel) / self.maxVelocity
        
        # Forces are normalized using the maxmimum allowable force applied.
        extForcesX = np.abs(self.extForcesX) / self.maxAppliedForce
        extForcesY = np.abs(self.extForcesY) / self.maxAppliedForce
        extForces = np.concatenate((extForcesX, extForcesY))
        
        observation = np.concatenate((botPos.flatten(), botVel.flatten(), botForces, extForces, runTime))
        observation = np.concatenate((observation, squareObsLocation, squareVel)) # Adding square information
    ...
    def reportContact(self, contactPair, impulse):
        ...
        # Adding forces to square
        self.extForceSquare[0] = forces[0]
        self.extForceSquare[1] = forces[1]
    ...
    def calcRew(self, distanceToTarget, distanceToSquare):
        ...
        - distanceToSquare: The distance of the system center to the square location
        """
        #### Reward the system for getting the square closer to the center
        ...
        # rew = progress*1000 + closer
        # rew = closer
        

        #### Reward the system for getting as many bots in contact with the object as possible
        ...
        #### Reward the system for applying a force on object, but 0 net force overall!
        extForces = np.abs(np.vstack((self.extForcesX, self.extForcesY)))
        l2_F = np.linalg.norm(extForces, axis=0)
        l2_sum = np.sum(l2_F) 
        sum_F = np.sum(extForces, axis=1)
