...
The goal of this environment is to rotate the box (which is stuck in place) to its proper orientation.
"""
...
class pymunkEnv(Env):
    ...
    def __init__(self, dt, ppm, screenHeight, screenWidth, 
        ...
        self.maxVelocity = 5                                         # Arbitrarily set, may need changing later.
        self.maxAppliedForce = 1.0                                   # Defining the maximum force a bot can apply in any given direction
        ...
        self.start_options = [
            ...
            [self.width/2 - (R + self.squareLength + botRadius), self.height/2]#,  # Left side of box
            # [self.width/2 + (R + self.squareLength + botRadius), self.height/2]     # Right side of box
        ]
        ...
        #### Gym API
        self.state_size = self.numBots*8 + 1  # botRePosition (x,y), botVel (x,y), botLastAction(x,y), botExternalForce (x,y), orientation of box
        ...
        self.observation_space = spaces.Box(low, high, 
                                            shape=(numBots*8 + 1,), # [Posx, Posy, Velx, Vely, Ax, Ay, Fx, Fy] + object orientation
                                            dtype=np.float32)
    ...
    def step(self, ac):
        ...
        forcesX = forcesX_ANN + potFx*5
        forcesY = forcesY_ANN + potFy*5
    ...
    def getOb(self, ac):
        ...
        squareTheta = self.square.body.angle / 2*np.pi                       # Normalized rotation of square
        ...
        extForcesX = np.abs(self.extForcesX) / (5*self.maxAppliedForce)
        extForcesY = np.abs(self.extForcesY) / (5*self.maxAppliedForce)
        extForces = np.vstack((extForcesX, extForcesY)).T
        
        # Adding Bot information to observation
        n = self.numBots
        observation[0:2*n] = botPos.flatten()     # Positions
        observation[2*n:4*n] = botVel.flatten()     # Velocities
        observation[4*n:6*n] = extForces.flatten()  # External forces on bots
        observation[6*n:8*n] = botForces.flatten()
        observation[-1] = squareTheta
    ...
    def calcRew(self, distanceToTarget, distanceToSquare):
        ...
        diff = self.square.body.angle - self.goal_orientation
        rew = 5*diff # Receives reward for spinning in one direction
        return rew
