...
class pymunkEnv(Env):
    ...
    def __init__(self, dt, ppm, screenHeight, screenWidth, maxNumSteps, R, 
        ...
        self.state_size = 2  # botRePosition (x,y)
        self.action_size = 2
        ...
        act_high = np.array([self.R*2, 2*np.pi])
    ...
    def reset(self):
        ...
        ac = np.array([0]*2) # Take no action
    ...
    def step(self, ac):
        ...
        dis = self.convert.Meters2Pixels(ac[0])
        angle = ac[1]
        ...
        for i in range(self.numStepsPerStep):
            ...
            # Get the center location of the system/
            bot_poses = np.zeros((self.numBots,2))
            for i, bot in enumerate(self.bots):
                pos = np.asarray(bot.body.position)
                bot_poses[i] = pos

            center = np.mean(bot_poses, axis=0)
            actLoc = np.array([center[0] + dis*cos(angle), center[1] + dis*sin(angle)])
            ...
            for j, bot in enumerate(self.bots):
                ...
                # Query the bot position
                pos = bot_poses[j]
                rel_loc = actLoc - pos 
                
                #Normalize the position
                rel_loc /= np.linalg.norm(rel_loc)

                # Get the forces
                xForce, yForce = self.maxAppliedForce * rel_loc
            ...
            for index, bot in enumerate(self.bots):
                ...
                botPos = bot_poses[index]
                bot.body.apply_force_at_world_point((xForce, yForce), (botPos[0], botPos[1]))
    ...
    def getOb(self, ac):
        ...
        observation = systemCenter # Observation is currently the system center.
    ...
    def calcRew(self, distanceToTarget, sysNormForce):
        ...
        rew = progress*2000 - sysNormForce*.5
        if rew<0: rew = 0 
        """
    ...
    def plot_data(self):
        ...
        # Plot Actions
        plt.figure('Agent_R')
        plt.plot(time, self.ac[:,1])
        plt.xlabel(xlabel)
        plt.ylabel('R [m]')
        plt.title('Distance from Center')
        plt.savefig(self.saveFolder +'agent_R.jpg')

        plt.figure('Agent_Theta')
        plt.plot(time, self.ac[:,2])
        plt.xlabel(xlabel)
        plt.ylabel('Theta [rad]')
        plt.title('Theta')
        plt.savefig(self.saveFolder +'agent_Theta.jpg')
...
def createObstacleField(systemStart, width, height, totalR, space, friction, wallThickness=0.5):
    """
    Inputs
    ...
    # Wall4-UP | (Going down!)
    # x4a, y4a = x3b, y3b
    # x4b, y4b = x4a, y4a - height/2
    # body4 = pymunk.Body(0,0,body_type=pymunk.Body.STATIC)
    # shape4 = pymunk.Segment(body4, (x4a,y4a),(x4b,y4b),wallThickness)
    # bodies.append(body4)
    # shapes.append(shape4)

    # Wall5-UP -- (Finishing to the right)
    # x5a, y5a = x4b, y4b
    # x5b, y5b = x5a + width/6, y5a
    # body5 = pymunk.Body(0,0,body_type=pymunk.Body.STATIC)
    # shape5 = pymunk.Segment(body5, (x5a,y5a),(x5b,y5b), wallThickness)
    # bodies.append(body5)
    ...
    # Wall9-Down | (Going back down!)
    # x9a, y9a = x8b, y8b
    # x9b, y9b = x9a, y9a - height/2
    # body9 = pymunk.Body(0,0,body_type=pymunk.Body.STATIC)
    # shape9 = pymunk.Segment(body9, (x9a,y9a), (x9b,y9b), wallThickness)
    # bodies.append(body9)
    # shapes.append(shape9)


    # Wall10-Down   -- (Going to the right, final wall)
    # x10a, y10a = x9b, y9b
    # x10b, y10b = x10a + width/6 + gapSize, y10a
    # body10 = pymunk.Body(0,0,body_type=pymunk.Body.STATIC)
    # shape10 = pymunk.Segment(body10, (x10a,y10a), (x10b,y10b), wallThickness)
    # bodies.append(body10)
    # shapes.append(shape10)
...
def createObstacleField2(systemStart, width, height, totalR, space, friction, wallThickness=0.5):
    """
    Allows for a smaller hole that we must tunnel through
