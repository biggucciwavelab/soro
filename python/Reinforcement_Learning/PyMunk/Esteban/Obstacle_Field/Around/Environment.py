...
class pymunkEnv(Env):
    ...
    def __init__(self, dt, ppm, screenHeight, screenWidth, maxNumSteps, R, 
        ...
        self.forceGain = 2.0                                     # The maximum force a bot can apply in any given direction
        ...
        self.state_size = self.numBots*2 # botRePosition (x,y) # , botVel (x,y), botCurrentAction(x,y), timeLeft
    ...
    def reset(self):
        ...
        # Place obstacle in front of system
        obstacleRadius = 2*R
        obstaclePosition = systemStart[0] + 2*botRadius + R + obstacleRadius, systemStart[1]
        obstacleKwargs = dict(
            space = self.space,
        ...
        obstacle = Obstacle(**obstacleKwargs)

        # Define the target location & distance
        targetLoc = np.array([obstaclePosition[0] + obstacleRadius + R, obstaclePosition[1]])
        self.targetLoc = self.convert.Pixels2Meters(targetLoc)
        self.targetDistance = np.linalg.norm(self.targetLoc - self.systemStart)
    ...
    def getOb(self, ac):
        ...
        observation = botPos.flatten() # Currently the only observation is based on position
    ...
    def calcRew(self, distanceToTarget, sysNormForce):
        ...
        rew = progress*100 - sysNormForce*.5
        if rew<0: rew = 0 
        """
