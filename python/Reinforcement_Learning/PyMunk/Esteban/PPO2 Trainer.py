...
# import Strings_Environment_Experiment_212 as Strings
...
if __name__ == '__main__':
    # This dictionary correlates how large the system is to the maximum number of timesteps it will be given to reach its target
    ...
    # In[Prepare Experiment Parameters]
        ...
        # Training Parameters
        neural_network =[500,400,300,200,100]
        policyName = 'CustomPolicy_'+str(experimentNum)
        gamma = 0.99            # Discount factor
        n_steps = 100          # Number of steps to run in each environment per update. Batchsize = n_steps*n_env
        ...
        tunnel = False
        energy = False       # Penalty for too much energy consumption in locomotion
        kineticEnergy = False # Penalty for low kinetic energy
        velocityPenalty = True# Penalty for not moving
        distanceReward = False # Penalty which decreases as the distance to the target is decreased
        ...
        # The below is used for the object starting on the left.
        ...
        # For normal start, use the below SystemStart location
        ...
        env = make_vec_env(Strings.pymunkEnv, n_envs= nEnvs, env_kwargs=envParams, vec_env_cls=SubprocVecEnv, monitor_dir = savefile)
        ...
        model = PPO2(policyName, env, verbose=1, 
                      ...
                      seed = seed)
        # model = PPO2.load('ppo2_Experiment_212.zip',env,tensorboard_log=savefile,verbose=1)
        ...
        model.save(savefile+'ppo2_' + experimentName)
        env.close()
        ...
        if test:
            ...
            model = PPO2.load('ppo2_' + experimentName+'.zip')
