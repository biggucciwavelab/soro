...
def constrain_boxes(space, b1, b2, friction):
    pivot = pymunk.PivotJoint(b1, b2, (0,0), (0,0))
    space.add(pivot)
    pivot.max_bias = 0 # Disable joint correction
    pivot.max_force = friction


def add_main_box(space, box_length, mass, friction):
    global width
    global height

    position = 7 + 3*box_length/2, height/2

    static_body = space.static_body

    body_main = pymunk.Body()
    space.add(body_main)
    body_main.position = position
    shape = pymunk.Poly.create_box(body_main, (box_length, box_length, 0.0))
    shape.mass = mass
    shape.friction = 0
    space.add(shape)

    # Constrain the bodies
    constrain_boxes(space, static_body, body_main, friction)

    return body_main


def add_control_box(space, box_length, mass):
    global width
    global height
    body = pymunk.Body()
...
def create_space(mass, box_length, fake_friction = np.inf):
    ...
    control_box = add_control_box(space, box_length, mass)
    main_box = add_main_box(space, box_length, mass, friction)
...
if __name__=="__main__":
    ...
    # Run simulation
    run_sim(space, width, height, numSteps, dt, control_box, main_box, force)