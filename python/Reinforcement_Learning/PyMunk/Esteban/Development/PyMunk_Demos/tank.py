...
def init():
    """
    Define emulalated linear and angular friction rates
    for testing

    Original values:
        linear_friction_bodies = 1_000 
        angular_friction_bodies = 5_000
        linear_friction_tank = 10_000
        angular_friction_tank = 50_000
    """
    linear_friction_bodies = 1000
    angular_friction_bodies = 5000
    linear_friction_tank = 10_000
    angular_friction_tank = 50_000
    ...
    for _ in range(50):
        ...
        pivot.max_force = linear_friction_bodies  # emulate linear friction

        gear = pymunk.GearJoint(static_body, body, 0.0, 1.0)
        space.add(gear)
        gear.max_bias = 0  # disable joint correction
        gear.max_force = angular_friction_bodies  # emulate angular friction
    ...
    tank_body.position = 320, 240 # Note how the tank_control_body and tank_body have the same starting location
    ...
    space.add(pivot)
    pivot.max_bias = 0  # disable joint correction
    pivot.max_force = linear_friction_tank # emulate linear friction
    ...
    gear.max_force = angular_friction_tank  # emulate angular friction
