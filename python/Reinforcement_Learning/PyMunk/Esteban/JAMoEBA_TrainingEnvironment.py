...
class pymunkEnv(Env):
    ...
    def __init__(self, dt, ppm, screenHeight, screenWidth, maxNumSteps, R, 
                 ...
                 targetDistance, dataCollect=False, experimentName="NOT NAMED", 
                 saveVideo = False, obstaclesPresent = False, grabObjectPresent=False, 
                 potFieldPresent=False, tunnel=False, energy=False, kineticEnergy = False, 
                 velocityPenalty=False, distanceReward=False, targetLoc = None, 
                 systemStart = None, numStepsPerStep=1):
        """
        ...
        self.distanceReward = distanceReward # Are we incorporating the distance to the target as part of the reward schema
        if self.distanceReward:
        ...
        self.environment_parameters = [['dt:',str(self.dt)],
                                       ...
                                       ['Target Distance:',str(targetDistance)],
                                       ['tunnel',str(tunnel)],
                                       ['energy',str(energy)],
    ...
    def step(self, ac):
        ...
        # If there is an object present, then we must calculate the distance to said object
        botDisToObj = 0
        if self.grabObjectPresent:
        ...
        rew = self.calcRew(distanceToTarget, botDisToObj, sysNormForce)
    ...
    def calcRew(self, distanceToTarget, botDisToObj, sysNormForce):
        ...
        rew = progress*2000 + botsToBallProgress*500 - sysNormForce*.5
        if rew<0: rew = 0 # Only reward for POSITIVE motion
...
class starObstacle:
    def __init__(self, space, position, width, friction, rotation=0, color=(0,0,0,255)):
        ...
        """
        Creating shape via polygon
        """
        # star = pymunk.Poly(self.body, polyPoints, radius=5)
        # star.collision_type = 1
        # star.friction = friction
        # star.color = color
        # space.add(star)
        ...
        #     seg = pymunk.Segment(self.body,(xo[point],yo[point]),(xi[point],yi[point]),5)
        #     seg.friction= friction
        #     seg.color = color
        #     seg.collision_type = 1
        #     segments.append(seg)
        ...
        #     if point==4:
        #         seg = pymunk.Segment(self.body,(xi[point],yi[point]),(xo[0],yo[0]),5)
        #         seg.friction = friction
        #         seg.color= color
        #         seg.collision_type = 1
        #         segments.append(seg)
...
def createObstacleField(space, obstacleCoordinates, obsRadius, friction, square=False, star=False):
        ...
        for coord in obstacleCoordinates:
            ...
            elif star: obstacle = starObstacle(space,(posX,posY),obsRadius,friction,(np.pi)*np.random.rand())
