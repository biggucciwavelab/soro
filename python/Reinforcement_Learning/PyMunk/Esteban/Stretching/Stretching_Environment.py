...
class pymunkEnv(Env):
    ...
    def __init__(self, dt, ppm, screenHeight, screenWidth, 
        ...
        #### Gym API
        self.state_size = self.numBots*8  # botRePosition (x,y), botVel (x,y), botCurrentAction(x,y), objectPos(x,y), botVel(x,y)
    ...
    def reset(self):
        ...
        self.bots, self.membrane, interiorParticles, self.bindingSprings, self.bindingSliders, self.bindingList \
            = createJamoeba(**kwargs)
        for constraint in self.bindingSliders[0]:
    ...
    def getOb(self, ac):
        ...
        observation = np.concatenate((botPos.flatten(), botVel.flatten(), botForces, extForces))
    ...
    def getConstraintForces(self):
        """
        This function will calculate the X and Y forces acting on each bot as a result of the constraints.
        """
        pass
...
def connectBalls(space, theta1, theta2, b1, b2, rest_length, spring_stiffness, spring_damping, maxSeparation):
    ...
    return springConstraint, slideJoint
...
def createJamoeba(space, systemCenterLocation, systemRadius, numBots, botMass, botRadius, skinMass, skinRadius, skinRatio, botFriction, springK, springB, springRL, maxSeparation, inRadius, inMass, inFriction,  percentInteriorRemove = 0, botCollisionIntStart = 2):
    ...
    bindingSliders = []    # Where we will store any slider joint attached to a bot. Ignoring those attached to skin particles.
    ...
    if not bots_only:  
        ...
        for index, body in enumerate(membrane):
            if index < (numBodies-1):
                springJoint, slideJoint = connectBalls(space, t*index, t*(index+1), body, membrane[index+1], springRL, springK, springB, maxSeparation)
                
            else:
                springJoint, slideJoint = connectBalls(space, t*index, 0, body, membrane[0], springRL, springK, springB, maxSeparation) # GOOD ONE
            springs.append(springJoint)
            sliders.append(slideJoint)
        ...
        sliders = np.asarray(sliders)[keep]
    ...
    return bots, membrane, interiorParticles, bindingSprings, bindingSliders, bindingList
