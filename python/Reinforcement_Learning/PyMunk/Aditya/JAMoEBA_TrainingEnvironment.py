# -*- coding: utf-8 -*-
"""
Created on Fri Mar 26 14:18:53 2021
...
class pymunkEnv(Env):
    ...
    def __init__(self, dt, ppm, screenHeight, screenWidth, maxNumSteps, R, numBots, botMass, botRadius, skinRadius, skinMass, skinRatio, inRadius, botFriction, inMass, inFriction, percentInteriorRemove, springK, springB, springRL, wallThickness, maxSeparation, targetDistance, dataCollect=False, experimentName="NOT NAMED", saveVideo = False, obstaclesPresent = False, grabObjectPresent=False, potFieldPresent=False, tunnel=False, targetLoc = None, systemStart = None):
        ...
        if self.grabObjectPresent:
            ...
            self.state_size = self.numBots*8+1 # botRePosition (x,y), botVel (x,y), botCurrentAction(x,y), timeLeft
    ...
    def reset(self):
        ...
        #Gonna  use  this  to  store  contact  info
        self.contactPointX = np.zeros(self.numBots)
        self.contactPointY = np.zeros(self.numBots)
    ...
    def step(self, ac):
        ...
        # Gather information
        obs, systemCenter, distanceToTarget, _ = self.getOb(ac)
        botDisToObj = 0
        if self.grabObjectPresent:
    ...
    def getOb(self, ac):
        ...
        #Merging  the  two  contact  arrays
        # contactPoint = np.concatenate(self.contactPointX, self.contactPointY)
    ...
    def reportContact(self, contactPair, impulse):
        ...
        # for bot in self.bots:
        #     self.contactPointX = self.convert.Pixels2Meters(bot.body.position.x)
        #     self.contactPointY = self.convert.Pixels2Meters(bot.body.position.y)
        #### Get  the  ID  of  the  bot  that  collides  with  the  object
        bot = botIndex - 2 
        print(bot.body.position.x)
        print(bot.body.position.y)
        # self.contactPointX = bot.body.position.x
        # self.contactPointY = bot.body.position.y
    ...
    def calcRew(self, distanceToTarget, botDisToObj, sysNormForce):
        ...
        power_used = df  #Using  just  the  force  to  calculate  the  reward
        ...
        rew = progress*2000 + botsToBallProgress*500 - power_used*200
    ...
    def dataCollection(self,ac,rew,obs):
        ...
        Contact1_temp = [self.time]
        Contact2_temp = [self.time]
        ...
        for bot in self.bots:
            ...
            #Creating  an  array  to  store  the  bot  lositions  at  the  time  of  contact
            currentBotContact = self.convert.Pixels2Meters(bot.collide.point_a), self.convert.Pixels2Meters(bot.collide.point_b)
            ...
            Contact1_temp.append(currentBotContact[0])
            Contact2_temp.append(currentBotContact[1])
        ...
        Contact1_temp = np.asarray(Contact1_temp)
        Contact2_temp = np.asarray(Contact2_temp)
        ...
        self.Contact1 = np.vstack([self.Contact1, Contact1_temp])
        self.Contact2 = np.vstack([self.Contact2, Contact2_temp])
    ...
    def dataExport(self):
        ...
        self.Contact1 = np.delete(self.Contact1, 0, 0)
        self.Contact2 = np.delete(self.Contact2, 0, 0)
        ...
        np.savetxt(self.saveFolder + 'Contact1.csv', self.Contact1, delimiter=',')
        np.savetxt(self.saveFolder + 'Contact2.csv', self.Contact2, delimiter=',')
