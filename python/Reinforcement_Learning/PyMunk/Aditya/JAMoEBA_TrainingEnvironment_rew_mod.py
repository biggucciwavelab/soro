...
class pymunkEnv(Env):
    ...
    def reset(self):
        ...
        observation, self.previousDistance,_ = self.getOb(ac)
        return observation
    ...
    def step(self, ac):
        ...
        self.previousDistance = systemCenter
    ...
    def calcRew(self, distanceToTarget,systemCenter):
        progress = self.previousDistance - systemCenter # Relative to the velocity or speed for arriving at target
        
        closer = ((self.targetDistance/self.targetDistance) - (distanceToTarget/self.targetDistance))*10

        rew = closer + progress*.1
        return rew
...
def createJamoeba(space, systemCenterLocation, systemRadius, numBots, botMass, botRadius, skinMass, skinRadius, skinRatio, botFriction, springK, springB, springRL, maxSeparation, inRadius, inMass, inFriction,  percentInteriorRemove = 0):
...
def createWalls(space, screenHeight, screenWidth, wallThickness):
    ...
    vs1 = [(screenWidth//2,screenHeight),((screenWidth),screenHeight),((screenWidth//2)+wallThickness*8,(screenHeight//2)+(wallThickness*3)),((screenWidth),(screenHeight//2)+(wallThickness*3))]
    ...
    vs2 = [(screenWidth//2,0),((screenWidth),0),((screenWidth//2)+wallThickness*8,(screenHeight//2)-(wallThickness*3)),((screenWidth),(screenHeight//2)-(wallThickness*3))]
    ...
    # obst_shape3 = pymunk.Poly.create_box(obst3,(wallThickness, screenWidth//2))
    # obst3.body.position = ((screenWidth//2)-wallThickness*10, screenHeight//2-wallThickness*3)
    
    # #Obstacle  4
    # obst4 = pymunk.Body(0,0,body_type=pymunk.Body.STATIC)
    # obst_shape4 = pymunk.Poly.create_box(obst4,(wallThickness, screenWidth//2))
    # obst4.body.position = ((screenWidth//2)-wallThickness*10, (screenHeight//2)+wallThickness*3)
    
    #Adding  the  obstacle  to  the  observation  space
    
    space.add(shape1,body1,shape2,body2,shape3,body3,obst_shape1,obst1,obst_shape2,obst2)
    
    return shape1, shape2, shape3

# def createTunnel(space, screenHeight, screenWidth):
#     #All  values  assumed  to  be  in  pixels
    
#     #Bottom  Tunnel
#     tunnel_body1 = pymunk.Body(0,0,body_type = pymunk.Body.STATIC)
#     shape1 = pymunk.Poly.create_box(tunnel_body1, (screenWidth//2,(screenHeight//2)-30))
#     shape1.body.position = (screenWidth//2, screenHeight//2)
    
#     tunnel_body2 = pymunk.Body(0,0,body_type = pymunk.Body.STATIC)
#     shape2 = pymunk.Poly.create_box(tunnel_body2, (screenWidth//2, (screenHeight//2)+30))
#     shape2.body.position = (screenWidth//2, screenHeight//2)
...
#     obst1 = pymunk.Body(0,0,body_type = pymunk.Body.STATIC)
#     obst_shape1 = pymunk.Circle(obst1,1)
#     obst_shape1.body.position = (screenWidth//2,screenHeight//2)
    
#     #only  one  center  piece  obstacle
#     obst2 = pymunk.Body(0,0,body_type = pymunk.Body.STATIC)
#     obst_shape2 = pymunk.Circle(obst2,1)
#     obst_shape2.body.position = (screenWidth//2,screenHeight//2)
