...
class robot:
    ...
    def __init__(self, dt, ppm, numStepsPerStep, screenHeight, screenWidth, maxNumSteps, 
                 ...
                 springK, springB, springRL, maxSeparation, minSeparation,
                 ...
                 saveVideo=False, large_map=False):
        """
        ...
        self.height= self.convert.Pixels2Meters(screenHeight)        # Height of the screen, in meters
        self.width = self.convert.Pixels2Meters(screenWidth)         # Width of the screen, in meters
        self.screenWidth = screenWidth                               # Width of the screen, in meters
        self.screenHeight = screenHeight                             # Height of the screen, in meters
        self.saveVideo = saveVideo
        self.experimentName = experimentName                         # Experiment name. Is assigned to plots folder and video

        # Distance sensors
        self.background = THECOLORS['white']#THECOLORS['grey']                          # Background color of simulation
        self.wall_color = (67, 1, 84, 255) # (149, 165, 166, 255)
        ...
        self.distance_reading_max = 2.5#botRadius * 4 * numBots # How far the distance sensors can read to
        self.num_points = 500 # Number of sensor points to get a measurement from
        ...
        self.state_size = 8*numBots # Bot pos (x,y), Vel (x,y), Forces Applied (x,y), External Forces Seen (x,y)
        ...
        # Ratios of the environment with respect to environment radius
        self.cube_L_ratio = 2.001996  #* 1.1 # Ratio of the longest dimension of the cube to the system radius, R.
        self.cube_S_ratio = 0.415799 #*1.1# Ratio of the shortest dimension of the cube to the system radius, R.
        ...
        cube_L = self.cube_L_ratio*(R + .058/2) # Longest dimension of cube
        cube_S = self.cube_S_ratio*(R + .058/2) # Shortest dimension of cube
        x = self.width - (cube_S/2 + cube_L/2)
        y = self.height - (cube_S/2 + cube_L/2)
        self.systemStart = np.array([x, y])

        # Verify that the rendering height and width are large enough
        min_width = cube_S + 3*cube_L
        min_height = 2*cube_L - cube_S
        assert self.width>min_width, "\n\nWARNING!!! The screen's width is not large enough"
        assert self.height>min_height, "\n\n WARNING!! The screen's height is not large enough."
    ...
    def reset(self):
        ...
        systemStart = self.convert.Meters2Pixels(self.systemStart)
        ...
        self.bots, interiorParticles, self.membrane, self.constraints, self.distances, self.bars = createJamoeba(**kwargs)
        ...
        obstacleKwargs = dict(
            ...
            R=R*1.05+self.convert.Meters2Pixels(.058/2) * 1.5 * 1.05, #botRadius*1.5,
            cube_L_ratio=self.cube_L_ratio,
            cube_S_ratio=self.cube_S_ratio,
            large_map=self.large_map,
    ...
    def step(self, ac):
        ...
        for i in range(self.numStepsPerStep):
            for index, bot in enumerate(self.bots):
                ...
                xDir = cos(botAngle)
                yDir = sin(botAngle)
                bot.body.apply_force_at_world_point((force[0], force[1]), (botPos.x, botPos.y))
    ...
    def getOb(self):
        ...
        distanceTypes = np.zeros((self.numBots,1))

        for index, (bot, bar) in enumerate(zip(self.bots, self.bars)):
            ...
            try:
                distanceReading[index], distanceTypes[index] = self.get_sonar_reading(pos, currentBotAng, index)
                #distanceReading[index] -= self.botRadius
            except:
                distanceReading[index]= 100000
                distanceTypes[index] = 0


            if index % 2 == 0:

                if distanceTypes[index] == 1:
                    bot.shape.color = (27, 136, 20, 255)
                    bar.shape.color = (11, 90, 6, 255)

                    if distanceReading[index] < .05:
                        #bot.shape.color = (143, 138, 0, 255)
                        bar.shape.color = (117, 114, 5, 255)

                elif distanceTypes[index] == 2:
                    bot.shape.color = (151, 0, 0, 255)
                    bar.shape.color = (95, 0, 0, 255)


            else:
                if distanceTypes[index] == 1:
                    bot.shape.color = (77, 186, 70, 255)
                    bar.shape.color = (61, 140, 56, 255)
                elif distanceTypes[index] == 2:
                    bot.shape.color = (201, 47, 47, 255)
                    bar.shape.color = (145, 45, 45, 255)

            if index == 0:
                bot.shape.color = (255, 255, 255, 255)
        ...
        observation = np.hstack((botPos, botVel, botAng, distanceReading, distanceTypes))
    ...
    def get_sonar_reading(self, pos, angle, index):
        ...
        point, type_obj = self.get_arm_distance(arm_middle, x, y, angle, 0)
    ...
    def get_arm_distance(self, arm, x, y, angle, offset, refined=False):
        ...
        for point in arm:
            ...
            if rotated_p[0] <= 0 or rotated_p[1] <= 0:
                return point, tmp  # Sensor is off the screen.
            else:
                obs = self.screen.get_at(rotated_p)
                tmp = self.get_track_or_not(obs)
                if tmp != 0:
                    ...
                    if not refined:
                        ...
                        point, tmp = self.get_arm_distance(new_arm, x, y, angle,0, refined=True)
                        return point, tmp
                    else:
                        return point, tmp
    ...
    def render(self):
        ...
        if self.show_membrane:
            self.draw_membrane()
        pygame.display.update()
        if self.saveVideo and self.timestep % 4 == 0: # Can edit this so you are not saving images with as high a frequency
    ...
    def dataExport(self):
        ...
        np.savetxt(self.saveFolder + 'X_data.csv', self.X_data, delimiter=',', fmt='%1.5e')
        np.savetxt(self.saveFolder + 'X_vel_data.csv', self.X_vel_data, delimiter=',', fmt='%1.5e')
        np.savetxt(self.saveFolder + 'Y_data.csv', self.Y_data, delimiter=',', fmt='%1.5e')
        np.savetxt(self.saveFolder + 'Y_vel_data.csv', self.Y_vel_data, delimiter=',', fmt='%1.5e')
        #np.savetxt(self.saveFolder + 'actions.csv', self.ac, delimiter=',', fmt='%1.5e')
        np.savetxt(self.saveFolder + 'observations.csv', self.obs_data, delimiter=',', fmt='%1.5e')
...
class Bar:
    def __init__(self, space, body, position, length, radius, mass, friction,
        ...
        a = (-length * np.cos(theta), -length * np.sin(theta))
        b =  (length * np.cos(theta), length * np.sin(theta))
...
def connectBalls(space, b1, b2, rest_length, spring_stiffness,
                 ...
                 thetaPlus=[0,0], twoSprings=False, revolute_joint = False, revolute_joint_bar=False, robotRadius=1.):
    """
    ...
    if not revolute_joint:
        ...
        if not twoSprings: # Standard spring creation
            ...
            springConstraint2 = pymunk.DampedSpring(b1.body, b2.body,
                                                    anchor_a2,anchor_b2,
                                                    rest_length, spring_stiffness*20,
                                                    spring_damping)
        ...
        n1 = np.asarray(b1.body.rotation_vector)
        n2 = np.asarray(b2.body.rotation_vector)

        c0 = ((p1 - robotRadius * n1) + (p2 - robotRadius * n2)) / 2

        nm = n1 + n2
        nm /= np.linalg.norm(nm)

        angdiff = np.arccos(np.abs(np.dot(n1, n2)))

        c = c0 + nm * robotRadius / np.cos(.5 * angdiff)
        ...
        # Add bar
        bar = Bar(space, b1.body, (c[0], c[1] - 50), 42 * 3 / 6, 16 * 3 / 6, 0, 0, theta=np.pi/2 + thetaPlus[1], color=(0, 100, 0, 255))


        rest_length = .5 * (b2.body.angle - b1.body.angle) #np.linalg.norm(np.subtract(p1, p2))
        if rest_length < -np.pi/2:
            rest_length += np.pi
        if rest_length > np.pi/2:
            rest_length -= np.pi
        ...
        print('Rest length: ' + str(rest_length))
        ...
        ang2 = b2.body.angle
        if ang1 < ang2:
            springConstraint4 = pymunk.DampedRotarySpring(b1.body, b2.body, 0, spring_stiffness, spring_damping)
        else:
            springConstraint4 = pymunk.DampedRotarySpring(b1.body, b2.body, 2 * np.pi, spring_stiffness,
                                                          spring_damping)
        ...
        # Add a virtual spring, just so we can show the membrane
    ...
    return springConstraint, dis, bar
...
def createJamoeba(space, systemCenterLocation, systemRadius, numBots, botMass,
    ...
    revolute_joint: Whether to connect the joints using pivot joints
    """
    ...
    for i in range(numBots):
        theta = i*2*np.pi/numBots + np.pi / 2
    ...
    if useHeadings:
        ...
        for index, body in enumerate(membrane):
            if index < (numBodies-1):
                ...
                c, dis, bar = connectBalls(space, body,
                ...
                for k, w in enumerate(isBot):
                    if w: # This is True if it is a bot
                        ...
                        thetaPlus[k] = botHeadings[whichHeading]
                c, dis, bar = connectBalls(space, body, membrane[0], springRL,
    ...
    # Connect the balls normally
        ...
        for index, body in enumerate(membrane):
            if index < (numBodies-1):
                c, dis, bar = connectBalls(space, body, membrane[index+1], springRL, springK, springB, maxSeparation,
                                      minSeparation, twoSprings=twoSprings, revolute_joint=revolute_joint,
                                      robotRadius=systemRadius)
                constraints.append(c)
                distances.append(dis)
                bars.append(bar)
                ...
                c, dis, bar = connectBalls(space, body, membrane[0], springRL, springK, springB, maxSeparation, minSeparation,
                                      twoSprings=twoSprings, revolute_joint=revolute_joint, robotRadius=systemRadius)
                constraints.append(c)
                distances.append(dis)
                bars.append(bar)
    ...
    return bots, interiorParticles, membrane, constraints, distances, bars
...
def createObstacles(space, systemStart, R, cube_L_ratio, cube_S_ratio, large_map, color=(67, 1, 84, 255)):
    """
    The obstacles will be created and situated with respect to the systems start location
    """
    # Get the cube dimensions
    cube_L = R*cube_L_ratio
    cube_S = R*cube_S_ratio

    # Get master X,Y positions
    # Note that we shift everything slightly so the top right corner of the screen is correctly aligned now
    X,Y = systemStart
    X += cube_L/2 + cube_S/2
    Y += cube_L/2 + cube_S/2

    # Store shapes and bodies in a list to add them all to the space at the end
    objects_list = []

    # Define the right-most cube
    x = X 
    y = Y + cube_S/2 - cube_L
    dim = cube_S, cube_L*2
    ...
    shape.color = color
    objects_list.extend((body,shape))

    # Define the top cube
    x = X - (cube_L*2.5+cube_S/2)
    y = Y 
    dim = cube_L*6, cube_S
    body = pymunk.Body(0,0,body_type=pymunk.Body.STATIC)
    shape = pymunk.Poly.create_box(body, dim)
    ...
    # Defiine the bottom cube

    x = X - (cube_L*1+cube_S/2)
    y = Y + cube_S - cube_L*2
    if large_map:
        dim  = cube_L*2, cube_S
    else:
        dim  = cube_L*2*2, cube_S
    body = pymunk.Body(0,0,body_type=pymunk.Body.STATIC)
    shape = pymunk.Poly.create_box(body, dim)
    shape.body.position = x,y
    shape.collision_type = 1
    ...
    # Left Obstacle
    x = X - 2*cube_L
    y = Y + cube_S/2 - 2*cube_L + cube_S + cube_L/2
    dim = cube_S, cube_L
    body = pymunk.Body(0,0,body_type=pymunk.Body.STATIC)
    shape = pymunk.Poly.create_box(body, dim)
    shape.body.position = x,y
    ...
    # First diagonal
    x = X - (cube_L * 1.9 + cube_S / 2)
    y = Y + cube_S - cube_L * 3.1
    dim = cube_L*1.2, cube_S
    body = pymunk.Body(0, 0, body_type=pymunk.Body.STATIC)
    shape = pymunk.Poly.create_box(body, dim)
    shape.body.position = x, y
    shape.body.angle=np.pi/4
    shape.collision_type = 1
    shape.color = color
    objects_list.extend((body, shape))

    # Bottom obstacle, 2
    x = X - (cube_L * 2.9 + cube_S / 2)
    y = Y + cube_S - cube_L * 3.5
    dim = cube_L*1.4, cube_S
    body = pymunk.Body(0, 0, body_type=pymunk.Body.STATIC)
    shape = pymunk.Poly.create_box(body, dim)
    shape.body.position = x, y
    shape.collision_type = 1
    ...
    # Top diagonal
    x = X - (cube_L * 1.15 + cube_S / 2)
    y = Y + cube_S*2.5 - cube_L * 3
    dim = cube_L * .8, cube_S
    body = pymunk.Body(0, 0, body_type=pymunk.Body.STATIC)
    shape = pymunk.Poly.create_box(body, dim)
    shape.body.position = x, y
    ...
    # Corner diagonal
    x = X - (cube_L * .5 + cube_S / 2)
    y = Y + cube_S - cube_L * 4
    dim = cube_L * 1.7, cube_S
    body = pymunk.Body(0, 0, body_type=pymunk.Body.STATIC)
    shape = pymunk.Poly.create_box(body, dim)
    shape.body.position = x, y
    shape.body.angle = np.pi / 4
    ...
    for i in range(2):
        # Circle obstacle
        x = X - (cube_L*3.65 )
        y = Y + - cube_L*2.9 + cube_L*1.5*i
        dim = cube_S * 2.5
        body = pymunk.Body(0, 0, body_type=pymunk.Body.STATIC)
        shape = pymunk.Circle(body, dim)
        shape.body.position = x, y
        shape.body.angle = np.pi
        shape.collision_type = 1
        shape.color = color
        objects_list.extend((body, shape))
