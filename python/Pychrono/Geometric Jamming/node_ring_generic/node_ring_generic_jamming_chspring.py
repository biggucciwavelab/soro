...
def main(
    ...
    x_o = 0.049 if(jammed) else 0.049
    cam_y = 3*R_o
    ...
    for body in ([ground] + bodies + springs): #([block_fixed, block_moving, link, ground] + bodies + springs):
        my_system.Add(body)
    ...
    csv_filename = 'results/Nsides_%d_Nnodes_%d_jamming_%i.csv' % (N_s, N_n, int(time.time()))
    ...
    if m_visualization == "irrlicht":
        ...
        while (myapplication.GetDevice().run()):
            ...
            #if (t > t_delay):
               # my_system.Set_G_acc(chrono.ChVectorD(1, -9.8, 0))
            if i % 100 == 0:
                print(t)
...
def get_ground(x_s, y_s, z_s, density=100, texture=None):
...
def get_block_fixed(R_o, x_s, y_s, z_s, density=1000, texture=None):
...
def get_block_moving(R_o, x_s, y_s, z_s, density=1000, texture=None):
