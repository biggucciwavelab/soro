...
def main(
    ...
    bodies = get_spiral(N_s=N_s, r_i=r_i, r_o=r_o, h=h, density=2414.3, N_n=N_n, texture=polygon_texture)
    springs = get_spiral_springs(bodies, x_o, r, k)
...
def get_spiral(N_s=8, r_i=0.025, r_o =0.027, h=0.01, density=2414.3, N_n=8, texture=None):
    spiral_bodies = []
    theta_sum = 2*N_n*np.pi
    r_inc = 2*r_o
    iter = r_inc/theta_sum
    theta = 0
    while theta <= theta_sum:
        dist = iter*theta
        ang = theta+2*pi
        x = dist*np.cos(ang)
        y = dist*np.sin(ang)
    ...
    for i in range(N_n):
        for j in range(N_n):
            ...
            spiral_bodies.append(body)
    return spiral_bodies

def get_spiral_springs(bodies, d=0.049, r=0.1, k=100):
    springs = []
    for body_A, body_B in zip(bodies[0:-1], bodies[1:]):
