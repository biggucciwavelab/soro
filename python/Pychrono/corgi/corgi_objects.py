...
def Square_BOTS_Spinners(nb,i,x,y,z,length,height,theta,rowr,body_floor,obj,my_system,material,Springs,k,rl,botcall,force):
    # Create bots    
    bot = chrono.ChBody()
    bot = chrono.ChBodyEasyBox(length,length,height,rowr)
    bot.SetPos(chrono.ChVectorD(x,y,z))
    bot.SetMaterialSurface(material)
    # rotate them
    rotation1 = chrono.ChQuaternionD()
    ...
    # collision model
    bot.GetCollisionModel().ClearModel()
    bot.GetCollisionModel().AddBox(length/2,length/2,height/2) # hemi sizes
    bot.GetCollisionModel().BuildModel()
    bot.SetCollide(True)
    bot.SetBodyFixed(False)
    pt=chrono.ChLinkMatePlane()
    ...
    obj.append(bot)
    return(obj,bot,my_system,Springs)
# In[Bots Spinners] for corgi model and others like it 
...
#Create empty contact matrices
    ...
    return(qx,qy,qz,rot0,rot1,rot2,rot3,Fxt,Fyt,Fzt,Xv,Yv,Zv,SL,Fmem,VNX,VNZ,VTX,VTZ,Fcx,Fcy,Fcz,xc,yc,zc)
