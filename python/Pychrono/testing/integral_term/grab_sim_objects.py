...
# In[Robot object]
class robot:
    def __init__(self,nb,diameter,height,rowr,material,k,rl,body_floor,my_system,fixed,type_spring,obj,mag,R,active,actbots,pathind,geom):
        # number of interior
        self.nb=nb 
        # diameter
...
class Controls:
    ...
    def __init__(self,forces,bots,interiors,fbound,Springs,my_system,k,rl,rlmax,type_spring,mag,nb,actbots,active,control_type,balls,mag_tan,mag_n,*args,**kwargs):
        # objects
        self.forces=forces
...
################################################################################################################                
    ...
    def leader_controller(self):
        ...
        mag_norm = self.mag_n*abs(l**2)-int_mag
        #mag_norm=0        
        # Convert mag_tan and mag_norm to Fx and Fz: basically proportional control based on distance and forward speed
        fx = self.mag_tan*tan_err[0] - mag_norm*self.norm_dir[0]
        fz = self.mag_tan*tan_err[1] - mag_norm*self.norm_dir[1]
        fx=np.array([fx])
        fz=np.array([fz])
        FX=[]
        FZ=[]
    ...
    def follower_controller(self):
        ...
        for i in range(self.nb-1):
            ...
            if self.poseind[i]==1:
                ...
                vel_err = self.vref-vel
                # PD control force based on position and speed

                fx=self.kp*pos_err[0]+self.kd*vel_err[0]
                fz=self.kp*pos_err[1]+self.kd*vel_err[1]
    ...
    def setspring(self):
        for i in range(len(self.Springs)):
            ...
            if var1>self.rlmax:
                ...
                if self.type_spring=="var":
                    self.Springs[i].Set_SpringK(10*self.k)
...
# In[Enviroment]            
class enviroment:
    def __init__(self,my_system,material,length,tall,env_mode=None):
        self.my_system=my_system
        self.material=material
        self.length=length
        self.tall=tall
        ...
        body_floor.SetPos(chrono.ChVectorD(0, -self.tall, 0 ))
        body_floor.SetMaterialSurface(self.material)
        body_floor.GetCollisionModel().ClearModel()
        body_floor.GetCollisionModel().AddBox(self.length, self.tall, self.length) # hemi sizes
        ...
        body_floor.AddAsset(col_g)
        self.my_system.Add(body_floor)
...
class simulate:
    ...
    def simulate(self):
        ...
        if self.visual=="pov":  
            ...
            pov_exporter.SetCamera(chrono.ChVectorD(4,5,0), chrono.ChVectorD(4,0,0,),90)
...
# Fill array forces
        ...
        if self.controller is not None:
            ...
            if self.controller.control_type=="grab_drag":
    ...
    def save_variables(self):
        ...
        with open(self.file_name18, 'w', newline='') as fout:
            ...
            if self.control_type=='path_following' or self.control_type=='grab_drag' or self.control_type=='grab_drag_A' or self.control_type=="tunneling":            
                w.writerow(['mag_tan', self.controller.mag_tan])
