# -*- coding: utf-8 -*-
"""
Created on Wed Feb  9 09:56:45 2022
...
class SpheroController:
    ...
    def __init__(self, single_sphero, kp=1., ki=0., kd=0., is_alone=True, method='pot'):
        ...
        if self.modes == 1:
            ...
            #if self.shape == 'Circle':
                #self.zz = self.potential_energy_donut()
            #    (self.x, self.y, self.z, self.x2, self.y2,
            #     self.z2) = self.Points_for_shape()
            # if its not a circle
            #else:
            (self.x, self.y, self.z, self.x2, self.y2,self.z2) = self.Points_for_shape()  # create points for the field
            #print('rbf1')
            self.rbf = Rbf(self.x, self.y, self.z,
                           function='thin_plate')  # create radial basis function 1 outer field
            #print('rbf2')
            self.rbf2 = Rbf(self.x2, self.y2, self.z2,
                            function='thin_plate')  # create radial basis function inner field
            ...
            #print('grade')
            self.X = None  # no obstacles
            self.Y = None  # no obstacles

        if self.modes == 2:
            self.X = [0]  # obstacle positions X
            self.Y = [0]  # obstacle position Y
            # self.df = pd.DataFrame(columns=['bot', 'time', 'x', 'y', 'vx', 'vy'])
            # self.file_name='data' + str(datetime.datetime.now())
            # if self.modes == 1:
            self.px = 0  # center x
            self.py = 0  # center y
            self.b = 400  # range of field
            self.res = 3  # resolution of the field so how big are the squares
            ...
            r = np.sqrt((self.xx - self.px) ** 2 + (self.yy - self.py) ** 2)  # r term for creating source field
            self.zz = r
            self.zz = self.zz / np.max(self.zz)  # normalize it
            (self.fy, self.fx) = np.gradient(self.zz)  # take gradient
            self.fx = self.fx / np.max(self.fx)  # normalize x gradient
            self.fy = self.fy / np.max(self.fy)  # normalize y gradient
        ...
        if self.modes == 3:
            ...
            self.px = 0  # center x
            self.py = 0  # center y
            self.b = 100  # range of field
            self.a = 3
            self.c = 3
            self.res = 1  # resolution of the field so how big are the squares
            self.xmin = self.px - self.b  # x max so how far to the right x
            self.xmax = self.px + self.b  # x min so how far to the left x
            self.ymin = self.py - self.b  # y max so how far to the right y
            self.ymax = self.py + self.b  # y min so how far to the left y
            ...
            self.yp = np.linspace(self.ymin, self.ymax, self.ycount)  # create y axis point
            self.xx, self.yy = np.meshgrid(self.xp, self.yp)  # create grid for the field
            self.zz = (np.sqrt(self.xx**2 / self.a**2 + self.yy**2 / self.c**2))**2 * np.log(np.sqrt(self.xx**2 / self.a**2 + self.yy**2 / self.c**2))
            self.zz = self.zz / np.max(self.zz)  # normalize it
    ...
    def potential_energy_donut(self):
        f1 = (self.xxt) ** 2 + (self.yyt) ** 2 - (self.r2) ** 2
        f2 = (self.r1) ** 2 - (self.xxt) ** 2 - (self.yyt) ** 2
        k1 = 1000
        p1 = (k1) * (np.maximum(0, f1))
        p1 = p1 / np.max(p1)
        p2 = (k1 / 2) * (np.maximum(0, f2) ** 2)
        p2 = p2 / np.max(p2)
        p = p1 + p2
        return (p)
    ...
    def points_square(self):
        ...
        # x points
        xt = np.array(
            [1, 1, 1, 1, 1, .75, .5, .25, 0, -.25, -.5, -.75, -1, -1, -1, -1, -1, -1, -1, -1, -1, -.75, -.5, -.25,
             0, .25, .5, .75, 1, 1, 1, 1])

        # y points
        yt = np.array(
            [0, .25, .5, .75, 1, 1, 1, 1, 1, 1, 1, 1, 1, .75, .5, .25, 0, -.25, -.5, -.75, -1, -1, -1, -1, -1, -1,
             -1, -1, -1, -.75, -.5, -.25])
    ...
    class controller:
        def __init__(self, px, py, zz, fx, fy, b, res, tstart,vmax):
            ...
            self.px = px  # center x
            self.py = py# center y
            #print('checking in',self.px,self.py)
        ...
        def out_vel1(self, x, y, vx, vy):
            ''' Controller for shape formation or mode 1 '''
    ...
    class controller2:
        def __init__(self, px, py,tstart,vmax):
        ...
        def out_vel2(self, x, y, vx, vy):
            ...
            theta = np.nan_to_num(180 * np.arctan2(Vy, Vx) / np.pi) % 360  # set heading angle
            #theta2 = signal.square(2 * np.pi * .2 * tt) * 5
            theta=theta#+theta2
            # print(self.r1,self.r2)
            return (V, theta)
        ...
        def out_vel5(self, x, y, vx, vy):
            ''' Controller for  grasping'''
            VY = self.F2y(x, y, self.px, self.py, 50, 50)
            VX = self.F2x(x, y, self.px, self.py, 50, 50)
            mag = np.sqrt(VY ** 2 + VX ** 2)  # magnitude
            VY = VY / mag  # normalize  Y
            VX = VX / mag  # normalize X
            Vx = -self.alpha * VX # - self.beta * vx
    ...
    def ros_callback(self, data):
        ...
        #if tt < 60:
        #    self.PotField = self.controller2(px, py, self.tstart, vmax=255)
        #    mag, ang = self.PotField.out_vel_4(self.x, self.y, 0, 0)
        #else:
        #    self.PotField = self.controller(px+30, py, self.zz, self.fx, self.fy, self.b, self.res, self.tstart, vmax=255)
        #    mag, ang = self.PotField.out_vel1(self.x, self.y, 0, 0)
        ...
        #print("\n Bot#",self.at_id,self.tar_x, self.tar_y, self.at_id, 'method:', self.control_method, "dist:",
