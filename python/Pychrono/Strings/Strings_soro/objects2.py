...
class Interiors:
    def __init__(self,name,my_system,body_floor,path):
        ...
        if self.interior_mode=="tunnel0" or self.interior_mode=="grasp0":
        ...
        elif self.interior_mode=="tunnel1" or self.interior_mode=="grasp1":
        ...
        elif self.interior_mode=="tunnel2" or self.interior_mode=="grasp2":
            self.total_particles = 24
            self.particle_material = self.Material(0.5)
        ...
        #### tunnel1 ####
        if self.interior_mode=="tunnel1" or self.interior_mode=="grasp1":
            count=0
        ...
        #### tunnel2 ####
        if self.interior_mode=="tunnel2" or self.interior_mode=="grasp2":
            count=0
...
class floor:
    def __init__(self,name,my_system,path):
        ...
        self.material1 = self.Material(0.04)
        self.material2 = self.Material(0)
        #self.material3 = self.Material(0.1)
        ...
        self.body_floor2.SetPos(chrono.ChVectorD(0, self.floor_height+.1025, 0 ))
...
class controller():
    ...
    def grasping_controller(self):
        ...
        if time<=self.Psi.tcut1:
            self.tarx = self.ball.balls[0].GetPos().x-.3
        ...
        if time>=self.Psi.tcut2:
            self.tarx2 = self.tarx+.3
            self.tarz2 = self.tarz        
        ...
        for i in range(self.nb):
            ...
            Fx2=self.Psi.FX_(100*self.bot_position_x[i],100*self.bot_position_z[i],xc=100*self.tarx2,yc=100*self.tarz2,a=100*self.Psi.a2,b=100*self.Psi.b2)
            Fz2=self.Psi.FY_(100*self.bot_position_x[i],100*self.bot_position_z[i],xc=100*self.tarx2,yc=100*self.tarz2,a=100*self.Psi.a2,b=100*self.Psi.b2)
            ...
            fx=-self.alpha1*FXX-self.beta1*self.bot_velocitiy_x[i]
            fz=-self.alpha1*FZZ-self.beta1*self.bot_velocitiy_z[i]
            FX.append(fx)
            FZ.append(fz)
        return(np.asarray(FX),np.asarray(FZ)) 
...
class import_data:
     ...
     def create_snap_shot_morph(self,entry,membrane):
        ''' Create snapshots  '''
        i=entry
        fig, ax = plt.subplots(figsize=(1.0,1.0),dpi=300,constrained_layout=True)
        plt.axis('off')
        ax = plt.axes(xlim=(self.wxmin,self.wxmax), ylim=(self.wymin, self.wymax))
        for j in range(0,self.nb):
            x0,y0=self.bot_position_x[j,i],self.bot_position_z[j,i]  
     ...
     def create_snap_shot(self,entry,membrane):
        ''' Create snapshots  '''
        i=entry
        fig, ax = plt.subplots(figsize=(1.5,1.5),dpi=300,constrained_layout=True)
        plt.axis('off')
        ax = plt.axes(xlim=(self.wxmin,self.wxmax), ylim=(self.wymin, self.wymax))
        for j in range(0,self.nb):
            x0,y0=self.bot_position_x[j,i],self.bot_position_z[j,i]  
