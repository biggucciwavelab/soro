...
class robot:
    def __init__(self,nb,radius,height,rowr,material,k,rl,body_floor,my_system,fixed,type_spring,obj,R,geom,xc,zc,skind,ratioM):
        ...
        self.skinrho=10000 # membrane density
        ...
        for i in range(self.nb):
            ...
            if self.geom=="cylinder":
                ...
                #     prismatic_ground = chrono.ChLinkLockPrismatic()
                #     prismatic_ground.SetName("prismatic_ground_ball")
                #     prismatic_ground.Initialize(self.body_floor, bot, chrono.ChCoordsysD(chrono.ChVectorD(x, y, z), chrono.Q_ROTATE_X_TO_Y))
                #     self.my_system.AddLink(prismatic_ground)
    ...
    def __init__(self,nb,radius,rowp,height,my_system,obj,body_floor,material,fixed,mode,R,xc,zc,n,sim):
        ...
        if self.mode=="anger":
            ...
            for i in range(self.ni):
                ...
                gran = chrono.ChBody()
                gran = chrono.ChBodyEasyCylinder(self.radius2-self.off, self.height,self.rowp,True,True)
                gran.SetPos(chrono.ChVectorD(xx[i]*.0254 + self.xc,y,zz[i]*.0254 + self.zc))
                gran.SetMaterialSurface(self.material)
                gran.SetName('gran'+str(con)+str(count))
                gran.SetId(i)
        ...
        if self.mode=="nonhnmax": 
            ...
            for i in range(len(self.n)):
                ...
                for j in range(self.n[i]):
                    ...
                    gran = chrono.ChBodyEasyCylinder(self.radius2/2-self.off, self.height,self.rowp,True,True)
...
class Ball:
    def __init__(self,control_type,my_system,body_floor,obj,material,height=None,R=None,rho=None,zball=None,xball=None):
        ...
        if self.geom=="star":
            ...
            # myforcez.SetMode(chrono.ChForce.FORCE) # set the mode
            # myforcez.SetMforce(100)
            # myforcez.SetDir(chrono.VECT_Z) # set direction 
            # myforcez.SetVpoint(chrono.ChVectorD(0,.06,0))
    ...
    def save_angle_data(self):
        ...
        self.bq2.append(q0)
        self.bq3.append(q0)            
        # self.bphi.append(np.arctan2(2*(q0*q1+q2*q3),1-2*(q1**2 +q2**2)))
        # self.btheta.append(math.asin(2*(q0*q2-q3*q1)))
        # self.bpsi.append(np.arctan2(2*(q0*q3+q1*q2),1-2*(q2**2 +q3**2)))        
...
class environment:
    ...
    def __init__(self,my_system,material,material2,length,tall,height2,env_mode=None,gapw=None,R=None,X=None,Z=None,px=None,py=None):
        ...
        if self.env_mode=='tunnel':    
            ...
            # # myforcez = chrono.ChForce() # create it 
            # # wall2.AddForce(myforcez) # apply it to bot object
            # # # myforcez.SetMode(chrono.ChForce.FORCE) # set the mode
            # # # myforcez.SetDir(chrono.VECT_Z) # set direction
            # # # myforcez.SetMforce(200)
            # # self.my_system.Add(wall2)   
            # # self.my_system.Add(prismatic_ground_wall)  
...
class Controls:
    ...
    def __init__(self,forces,bots,interiors,Springs,my_system,nb,control_type,balls,tstep,my_rep,w,tn,tpull,radius2,n,R,enviroment,forceb,boundary,*args,**kwargs):
       ...
       #### Import and set specific variables based on what control type we are running      
...
##############################################################################   
        ...
        if self.control_type=='pot_field_grab':
            ...
            #self.f=self.phi2.f     # potential field function
            #self.fnx=self.phi2.fnx # gradient in x direction  
            #self.fny=self.phi2.fny # gradient in the y direction             
...
##############################################################################   
        ...
        if self.control_type=='pot_field_grab_2':
            ...
            self.mode=0            
##############################################################################   
        ##### POT_FIELD_GRAB_3 PARAMETERS        
        if self.control_type=='grab_warp':
            self.Obst=self.enviroment.Obst
            self.phi=args[0][0]
            self.alpha=args[0][1] # proportional gain 
            self.beta=args[0][2]  # damping term 
...
# ##############################################################################            
        ...
        ##### SHAPE_WARP_complex PARAMETERS
        if self.control_type=='Complex_image_warp':
            self.phi=args[0][0]   # potential field object
            self.t=0
            self.alpha=args[0][1] # proportional gain 
            self.beta=args[0][2]  # damping term              
        ...
        ##### DOOR RIP    
        if self.control_type=="door_rip": 
            self.phi=args[0][0]     # potential field object (contains all the pot field stuff)  
            self.phi1 = self.phi[0]
            self.phi2 = self.phi[1]
            self.phi3 = self.phi[2]
            self.fny1=self.phi1.fny   # gradient in the y direction of field 
            self.fnx1=self.phi1.fnx   # gradinen in the x direction of field 
            self.fny2=self.phi2.fny   # gradient in the y direction of field 
            self.fnx2=self.phi2.fnx   # gradinen in the x direction of field 
        ...
        ##### IMPORT FIELD
        if self.control_type=="import_field": 
            self.phi=args[0][0]     # potential field object (contains all the pot field stuff)  
            self.fny=self.phi.fny   # gradient in the y direction of field 
            self.fnx=self.phi.fnx   # gradinen in the x direction of field 
            self.fnx=0
...
##############################################################################
    ...
    def run_controller(self):
        ...
        #### R-functions : Run the sR-functions controller 
        if self.control_type=="R-fields":
            (self.xb,self.zb)=self.get_position()
            (self.xbv,self.zbv)=self.get_velocity()
            (self.FX,self.FZ)=self.R_function_controller()
            self.apply_force(self.FX,self.FZ)
        ...
        if self.control_type=="grab_warp":
            (self.xb,self.zb)=self.get_position() # get the position of the bots
            (self.xbv,self.zbv)=self.get_velocity() # get the velocity of the bots
            (self.FX,self.FZ)=self.grab_controller3() # run the grab controller
            self.apply_force(self.FX,self.FZ) # apply the force to the bots    
        ...
        if self.control_type=="Complex_image_warp":
            (self.xb,self.zb)=self.get_position()
            (self.xbv,self.zbv)=self.get_velocity()
            (self.FX,self.FZ)=self.complex_warp_controller()
            self.apply_force(self.FX,self.FZ)  
        ...
        if self.control_type=="target_verify":
            ...
            self.apply_force(self.FX,self.FZ)  # apply the force to the bots            
        #### DOOR RIP : Run the DOOR rip controller 
        if self.control_type=="door_rip":
            (self.xb,self.zb)=self.get_position() # get the positions of the bots
            (self.xbv,self.zbv)=self.get_velocity() # get the velocities of the bots
            (self.FX,self.FZ)=self.door_rip_controller() # run the tunneling controller 
            self.apply_force(self.FX,self.FZ)  # apply the force to the bots

            
        #### IMPORT IMAGE : run the imported field controller   
        if self.control_type=="import_field":
            (self.xb,self.zb)=self.get_position()
            (self.xbv,self.zbv)=self.get_velocity()
            (self.FX,self.FZ)=self.shape_controller()
        ...
        #### VERIFY : Run the verification controller   
        if self.control_type=="Verify":
            (self.xb,self.zb)=self.get_position() # get the positions of the bots
            (self.xbv,self.zbv)=self.get_velocity() # get the velocities of the bots
            (self.FX,self.FZ)=self.Verify_controller() # run the tunneling controller 
        ...
        #### OBSTACLE : Run the tunneling controller     
        if self.control_type=="Obstacle":
            (self.xb,self.zb)=self.get_position() # get the positions of the bots
            (self.xbv,self.zbv)=self.get_velocity() # get the velocities of the bots
            (self.FX,self.FZ)=self.Obstacle_controller() # run the tunneling controller 
            self.apply_force(self.FX,self.FZ)  # apply the force to the bots
...
##############################################################################            
   ...
   #  Grabbing a ball controller 
    ...
    def grab_controller2(self):
        ...
        if t>10:
            self.balls.balls[0].SetBodyFixed(False)
            #self.phi.px=self.phi.px-.0005
            if abs(self.phi.px-self.balls.balls[0].GetPos().z)>.2:
                
                self.phi.px=self.phi.px
                self.fb=self.fb
            else:
                self.phi.px=self.phi.px-.0005
                self.fb=self.fb+.005
                
            self.balls.balls[0].SetMass(self.fb)    
            (self.f,self.fny,self.fnx)=self.phi.update_field_gradient()  
            print(np.round(self.phi.px,3),np.round(self.balls.balls[0].GetPos().z,3),np.round(self.fb,3))
        for i in range(self.nb):
            Fx=self.fny([self.xb[i],self.zb[i]])
            Fz=self.fnx([self.xb[i],self.zb[i]])
            mag=np.sqrt(Fx**2+Fz**2)
   ...
   #  Grabbing a ball controller 
    def grab_controller4(self):
        ...
        t=np.round(self.my_system.GetChTime(),4) 
        # if t>6:
        #     self.phi.a=(1-self.tanh(t-6))*4*self.phi.Rb + self.tanh(t-6)*self.phi.Rb
        #     self.phi.c=(1-self.tanh(t-6))*4*self.phi.Rb + self.tanh(t-6)*self.phi.Rb
    ...
    def grasp_verify_controller(self):
        ...
        if tt<8:
            ...
            self.alpha=0.15
            #self.phi2.py=self.balls.balls[0].GetPos().x
            #self.phi2.px=self.balls.balls[0].GetPos().z-0.5
            ...
            if tt>14:
                ...
                #self.phi2.py=self.balls.balls[0].GetPos().x
                #self.phi2.px=self.balls.balls[0].GetPos().z   
    ...
    def spring_controller(self):
        for i in range(len(self.boundary.Springs)):
            self.boundary.Springs[i].Set_SpringK(20)
            
    def grab_drag_controller(self):        
        ...
        FZ=[]
        print(self.cn)
        # if enough bots passed the ball
        if self.cond==True:
            self.alpha=20
            for i in range(self.nb):
                Fx=self.fny([self.xb[i],self.zb[i]])
        ...
        # if not enough passed the ball 
        if self.cond==False:
            dist=self.balls.radius*1.15
            dist2=self.balls.radius*.25
            for i in range(self.nb):
                ...
                if self.cn==self.num:
                    self.cond=True
            # if the robot gets to close turn off
                if abs(Mx)<abs(dist) and abs(Mz)<abs(dist) and self.cond==False:
                    fx=np.array([0])
                    fz=np.array([-.85*self.alpha])
                    FX.append(fx[0])
                    FZ.append(fz[0])   
    ...
    def grab_controller_warp(self):
        ...
        if t<1:
            for i in range(self.nb):
                Fx=self.phi1.Mx(self.tanh(t-self.tcut),self.zb[i],self.xb[i])
                Fz=self.phi1.My(self.tanh(t-self.tcut),self.zb[i],self.xb[i])            
                mag=np.sqrt(Fx**2+Fz**2)
                FXX=Fx/mag
                FZZ=Fz/mag
                fx=-self.alpha*FXX-self.beta*self.xbv[i]
            ...
            self.phi2.py=self.balls.balls[0].GetPos().x
            self.phi2.px=self.balls.balls[0].GetPos().z
            (self.f,self.fny,self.fnx)=self.phi2.update_field_gradient()
            FX=[]
            FZ=[]
            print(self.cn)
            # if enough bots passed the ball
    ...
    def grab_controller_warp_2(self):
        ''' This controller is used for grabbing a fixed object'''
        FX=[]
        FZ=[]
        t=np.round(self.my_system.GetChTime(),4) 
        if t<2:
            for i in range(self.nb):
                ...
                Fx=self.fny1([self.xb[i],self.zb[i]])
                Fz=self.fnx1([self.xb[i],self.zb[i]])
                mag=np.sqrt(Fx**2+Fz**2)
                if mag[0]==0:
                    FXX=np.array([0])
                    FZZ=np.array([0])
    ...
    #     FZ=[]
    #     t=np.round(self.my_system.GetChTime(),4) 
    #     print(self.tanh(t-self.tcut))
    #     if self.tanh(t-self.tcut)<.95:
    #         for i in range(self.nb):
    #             Fx=self.phi1.Mx(self.tanh(t-self.tcut),self.zb[i],self.xb[i],self.mode)
    #             Fz=self.phi1.My(self.tanh(t-self.tcut),self.zb[i],self.xb[i],self.mode)            
    #             mag=np.sqrt(Fx**2+Fz**2)
    #             FXX=Fx/mag
    #             FZZ=Fz/mag
    #             fx=-self.alpha*FXX-self.beta*self.xbv[i]
    ...
    #     else:
    #         self.phi1.px=1.5498171018279117
    #         print('party')
    #         self.phi1.update_field()
    #         print(self.phi1.px)
    #         for i in range(self.nb):
    #             Fx=self.phi1.Mx(1,self.zb[i],self.xb[i],self.mode)
    #             Fz=self.phi1.My(1,self.zb[i],self.xb[i],self.mode)            
    #             mag=np.sqrt(Fx**2+Fz**2)
    #             FXX=Fx/mag
    #             FZZ=Fz/mag
    #             fx=-self.alpha*FXX-self.beta*self.xbv[i]
...
##################################################################   
# controlling a shape  
    def R_function_controller(self):
            t=np.round(self.my_system.GetChTime(),4)
            phi1=self.phi[0]
            phi2=self.phi[1]
            FX=[]
            FZ=[]
            print('tanh',self.tanh(t))
            for i in range(self.nb):
                Fz=(1-self.tanh(t))*phi1.DFY(self.xb[i],self.zb[i],phi1.a,phi1.b,phi1.R) + self.tanh(t)*phi2.PHIDY(self.xb[i],self.zb[i],phi2.segments)
                Fx=(1-self.tanh(t))*phi1.DFX(self.xb[i],self.zb[i],phi1.a,phi1.b,phi1.R) + self.tanh(t)*phi2.PHIDX(self.xb[i],self.zb[i],phi2.segments)
    ...
    #     for i in range(self.nb):
    #         Fz=phi1.F()
    #         Fz=self.phi.PHIDY(self.xb[i],self.zb[i],self.phi2.segments)
    #         Fx=self.phi.PHIDX(self.xb[i],self.zb[i],self.phi.segments)
    #         mag=np.sqrt(Fx**2+Fz**2)
    #         if mag==0:
    #             FXX=np.array([0])
    #             FZZ=np.array([0])
    ...
    def Obstacle_controller(self):
        ...
        if self.count%2==0:
            ...
            for i in range(self.nb):
                FXB=[]
                FZB=[]
                for j in range(len(self.Obst)):
                    X=self.Obst[j].GetPos().x
                    Z=self.Obst[j].GetPos().z
                    dx=(self.xb[i] - X)
                    dz=(self.zb[i] - Z)
                    d = np.sqrt(dx ** 2 + dz ** 2)
                    d32= d** (-1.5)
                    ...
                    #     FXB.append(0)
                    #     FZB.append(0)
                    #print(np.nan_to_num(np.sum(FXB)))
                FXXX.append(np.nan_to_num(np.sum(FXB)))
                FZZZ.append(np.nan_to_num(np.sum(FZB)))
                
                    
        else:
            FXXX=np.zeros(self.nb)
            FZZZ=np.zeros(self.nb)
        ...
        for i in range(self.nb):
            ...
            FX.append(fx[0]+1*self.alpha*FXXX[i]+np.random.random_sample())
            FZ.append(fz[0]+1*self.alpha*FZZZ[i]+np.random.random_sample())
        
        return(np.asarray(FX),np.asarray(FZ))      
    
    
###################################################################################################    
    def door_rip_controller(self):
        FX=[]
        FZ=[]
        t = np.round(self.my_system.GetChTime(),4)
        if self.count%30==0:
            (self.xc,self.zc)=self.get_centroid()
        else:
            self.xc=self.xc
            self.zc=self.zc
        
        if self.xc>4 and self.trig!=0:
            self.trig=0
            self.tcut=t
            self.phi3.py=self.zc
            self.phi3.px=self.xc
        if self.xc>4:
            #self.alpha=30
            #print('party_time')
            print(self.tanh(t-self.tcut))
            for i in range(self.nb):
                Fx=self.phi3.Mx(self.tanh(t-self.tcut),self.zb[i],self.xb[i],self.mode)
                Fz=self.phi3.My(self.tanh(t-self.tcut),self.zb[i],self.xb[i],self.mode)
                mag=np.sqrt(Fx**2+Fz**2)
                if mag[0]==0:
                    FXX=np.array([0])
                    FZZ=np.array([0])  
    ...
    #     else:
    #         for i in range(self.nb):
    #             Fx=self.fny2([self.xb[i],self.zb[i]])
    #             Fz=self.fnx2([self.xb[i],self.zb[i]])
    #             mag=np.sqrt(Fx**2+Fz**2)
    #             if mag[0]==0:
    #                 FXX=np.array([0])
    #                 FZZ=np.array([0])
...
# point fields                    
class Obst_field:
    def __init__(self,px,py,res,b,X,Y):    
        self.type='point_field'
        self.px=px # center x
        self.py=py # center y
        self.b=b # range of field
...
# In[R functions class for creating field with series of lines ]
class series_of_lines:
    ...
    def PHI(self,x,y,segments):
        ...
        for i in range(len(segments[:,0])):
            R = R + 1/self.phi(x,y,segments[i,0],segments[i,2],segments[i,1],segments[i,3])**m
        R = 1/R**(1/m)
        return(R)
    ...
    def PHIDX(self,x,y,segments):
        ...
        term3=0
        for i in range(len(segments[:,0])):
            term1=-m*(self.phi(x,y,segments[i,0],segments[i,2],segments[i,1],segments[i,3])**-m) *self.delphix(x,y,segments[i,0],segments[i,2],segments[i,1],segments[i,3]) + term1
            term2=(self.phi(x,y,segments[i,0],segments[i,2],segments[i,1],segments[i,3])**-m)**(-1/m) + term2
            term3=m*(self.phi(x,y,segments[i,0],segments[i,2],segments[i,1],segments[i,3])**-m) + term3      
        R=(-term1*term2/term3)
        return(R)
    ...
    def PHIDY(self,x,y,segments):
        ...
        term3=0
        for i in range(len(segments[:,0])):
            term1=-m*(self.phi(x,y,segments[i,0],segments[i,2],segments[i,1],segments[i,3])**-m) *self.delphiy(x,y,segments[i,0],segments[i,2],segments[i,1],segments[i,3]) + term1
            term2=(self.phi(x,y,segments[i,0],segments[i,2],segments[i,1],segments[i,3])**-m)**(-1/m) + term2
            term3=m*(self.phi(x,y,segments[i,0],segments[i,2],segments[i,1],segments[i,3])**-m) + term3      
        R=(-term1*term2/term3)
        return(R)
...
class Circle_R_function:
    def __init__(self):
        self.a=0
        self.b=0
        self.R=0
    def F(self,x,y,a,b,R):
        phi=(R**2 - (x-a)**2 - (y-b)**2)/(2*R)
        return(abs(phi))
    def DFX(self,x,y,a,b,R):
        return(R*(x-a)*np.sign((R/2)*(- R**2 +(a-x)**2 + (b-y)**2)))
    def DFY(self,x,y,a,b,R):
