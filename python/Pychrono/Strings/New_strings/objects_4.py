...
class Interiors:
    def __init__(self,name,my_system,body_floor,path):
        ...
        self.particle_material = self.Material(0.3)
        data=np.load("F:/Soro_chrono/python/Pychrono/Strings/New_strings/target_track_start_position.npz")

        Rr=data['R']
        print(Rr)
        self.n=len(Rr)
        #print(count)
        self.total_particles=self.n
        
        self.parameters['n'] = self.n
        self.parameters['total_particles'] = self.total_particles
        self.parameters['Ri'] = Rr
...
class controller():
    ...
    def __init__(self,name,my_system,bots,Psi,path,ball=None,interior=None):
       ...
       # self.xc=1*data['Xc']
       # self.zc=1*data['Yc'] 
        data=np.load("F:/Soro_chrono/python/Pychrono/Strings/New_strings/Marker.npz")
        self.Xm=data['Xm']
        self.Ym=data['Ym']   
        self.Tm=data['time']  
        self.T=0
    def run_controller(self):
        ...
        if self.control_mode =="Verify":
                (self.bot_position_x,self.bot_position_z) = self.get_position()     # get position of bots
                (self.bot_velocitiy_x,self.bot_velocitiy_z) = self.get_velocity()   # get velocity of bots
                (self.FX,self.FZ) = self.Verify_controller() # run shape controller
    ...
    def Verify_controller(self):
        ...
        if tt<11:
            for i in range(self.nb):
                ...
                Fx=self.Fy(self.bot_position_z[i],self.bot_position_x[i],Y,X)
                Fz=self.Fx(self.bot_position_z[i],self.bot_position_x[i],Y,X)
                mag=np.sqrt(Fx**2+Fz**2)
            ...
            for i in range(len(self.interior.particles)):
                self.interior.particles[i].SetCollide(True)
                self.interior.particles[i].SetBodyFixed(False)            
    ...
    def change_marker(self,time):
        ...
        if len(res[0])==0:
            #print('na')
            x=self.ball.balls[0].GetPos().x
            z=self.ball.balls[0].GetPos().z
        else:
        ...
        #print(x,z)
            ...
            self.ball.balls[0].SetPos(chrono.ChVectorD(x,self.height/2,z))
        return(x,z)    
...
class import_data:
     ...
     def create_snap_shot(self,entry,membrane):
        ...
        plt.rcParams['axes.linewidth'] = .1
        plt.rcParams["text.usetex"] = True 
        fig, ax = plt.subplots(figsize=(0.9,0.9),dpi=300,constrained_layout=True)
        #plt.tight_layout()
        ...
        ax.plot(self.xp,self.yp,color='tab:red',linestyle='dashed',linewidth=0.5,zorder=0)
        plt.title('Time = ' + str(np.round(self.time[i],1))+" s",fontsize=6)
        #ax.axis('equal')
