...
def calculateGaps(obj):
    ...
    return g,ind
# In[Points for Grab]
def points_for_grab(p1,p2):
    theta1=np.linspace(np.pi/4,7*np.pi/4,50)
    x1=.75*np.cos(theta1)+p1
    y1=.75*np.sin(theta1)+p2

    theta2=np.linspace(np.pi/2,3*np.pi/2,20)
    x2=.5*np.cos(theta2)+.6+p1
    y2=.4*np.sin(theta2)+p2   
    ...
    x3=.5*np.cos(theta3)+1+p1
    y3=.4*np.sin(theta3)+p2
    ...
    x=np.concatenate((xp, xr), axis=None)
    y=np.concatenate((yp, yr), axis=None)
    z=np.concatenate((zp, zr), axis=None)
    
    return(x,y,z)
# In[Points for circle] 
def points_for_rbf_circle(nb,R,p1,p2,nr):
    theta=np.linspace(0,2*np.pi,nb)

    # create empty arays
    x=np.zeros(len(nr)*nb)
    y=np.zeros(len(nr)*nb)
    z=np.zeros(len(nr)*nb)
# create rings
    for j in range (len(nr)):
        for i in range(nb):
            nt=nr[j]
            x[i+nb*j]=(R+nt)*np.cos(theta[i])+p1
            y[i+nb*j]=(R+nt)*np.sin(theta[i])+p2
            if nt<1 and nt!=0:
                z[i+nb*j]=-1/nt
            if nt==0:
                z[i+nb*j]=0
...
def points_for_rbf_square(nb,R,p1,p2,nr):
...
def perimeter(xp,yp):
    ...
    for i in range(len(xp)-1):
        q=np.sqrt((xp[i+1]-xp[i])**2+(yp[i+1]-yp[i])**2)
        d.append(q)
        
    i=len(xp)-1
    q=np.sqrt((xp[0]-xp[i])**2+(yp[0]-yp[i])**2)
    d.append(q)
...
# In[Gradient RBF]
def gradient_RBF(br,bl,rbf):
    ti = np.linspace(bl, br, 100)
    xx, yy = np.meshgrid(ti, ti)
    zz = rbf(xx, yy)
    (zy,zx)=np.gradient(zz**2)
    # gradient of y
    fny = RegularGridInterpolator((ti,ti),zy)
    # gradient of x
    fnx = RegularGridInterpolator((ti,ti),zx)
    return(fny,fnx,zy,zx,xx,yy)

# In[Force on each bot]
def Force_on_bots_gradient(nb,xb,yb,fnx,fny):
    
    Fb=np.zeros(2*nb)
    for i in range(nb):
        Fb[2*i]=fnx([xb[i],yb[i]])
        Fb[2*i+1]=fny([xb[i],yb[i]])
    return(Fb)
  
# In[Apply force]
def Apply_force(Fb,nb,alpha,beta,j,F,vbar):
    for i in range(nb):
        F[3*i,j]=-(alpha*Fb[2*i+1])-beta*vbar[3*i]
        F[3*i+1,j]=-(alpha*Fb[2*i])-beta*vbar[3*i+1]
    return(F)
    

# In[Controller 2_5]
def Controller2_5(nb,qbar,j,F,fny,fnx,alpha,beta,rbf,vbar):
    #Feb=np.zeros(2*nb)
    x=[]
    y=[]
    for i in range(nb):
    ...
    xb=np.asarray(x)
    yb=np.asarray(y)
    Fb=Force_on_bots_gradient(nb,xb,yb,fnx,fny)
    (F)=Apply_force(Fb,nb,alpha,beta,j,F,vbar)
    #(ux,uy)=unit_norm(Fb,nb,alpha)
    
    #F=Apply_force(alpha,ux,uy,F,nb,j)
    return (F,Fb)    
# In[Error Check]
